...
class SAC:
    ...
    def _policy_loss_for(self, policy, q_function, q_function2, value_function):
        if not q_function2:
            if not self._reparameterize:
                ...
                # raise NotImplementedError
                actions, log_pis = policy(self._observations_ph)
                E_ea = tf.reduce_mean(self._alpha * policy(self._observations_ph) - q_function([self._observations_ph, actions]), axis=-1)
    ...
    def _value_function_loss_for(self, policy, q_function, q_function2, value_function):
        if not q_function2:
            ...
            ## E_sa.shape = |S|
            E_sa = tf.reduce_mean(q_function([self._observations_ph, self._actions_ph]) - self._alpha * log_pis, axis=-1)
