...
def build_critic(x, h, output_size, scope, n_layers, size, gru_size, recurrent=True, activation=tf.tanh, output_activation=None, regularizer=None):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        if recurrent:
            ...
            ## in build_rnn, somehow add logic "x = tf.reshape(x, (-1, x.get_shape()[1]*x.get_shape()[2]))", after dynamic_rnn(); keep gru_size for rnn
        else:
            x = tf.reshape(x, (-1, x.get_shape()[1]*x.get_shape()[2]))
        ...
        ## take care final x.shape = (?, output_size)
        # print("build_critic output x = ", x)
    return x
...
class Agent(object):
    ...
    def build_computation_graph(self):
        ...
        ## 1. critic update with recurrent paramaeter, hotfix from https://github.com/berkeleydeeprlcourse/homework/blob/master/hw5/meta/train_policy.py#L315
...
def main():
    ...
    parser.add_argument('--l2reg', '-reg', action='store_true', help="With --l2reg to activate l2 regularation, otherwise don't use l2 regularization")
    ## to parsing the recurrent flags, with --recurrent then set with arg.recurrent=True, otherwise without flag, then default value is False
    parser.add_argument('--recurrent', '-rec', action='store_true', help='With --recurrent to activate recurrent neural network, otherwise use mlp by default')
    ### process control
